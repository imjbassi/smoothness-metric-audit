...
under a diffusion policy, TED remains the worst condition, but the two
architectures do not agree on the rest: jerk, inert under behavior cloning,
becomes significantly \emph{better} than both baselines under diffusion,
so which metric a pipeline should trust is not a property of the metric
alone.
...
\subsection{Does the curation decision transfer across policy classes?}
\label{sec:crossarch}

The grid above uses one policy class, so it cannot separate a property of
the metric from a property of behavior cloning. We therefore replicated it
under a diffusion policy on the same curated subsets: same keep fraction,
same 1950-demonstration pools, same 600 epochs of 500 gradient steps, same
...
0.092 under diffusion), which is what one would expect if the deficit is a
property of the metric rather than of behavior cloning. But the harm is
statistically robust only under BC. Under diffusion the deficit is $-0.024$
against random and $-0.028$ against no curation, with intervals that admit
zero, and neither survives correction. This is a power statement, not a
...
falls below it. We therefore claim that TED's \emph{ordering} holds across
architectures and that its \emph{harm} is demonstrated under BC only.
...
under BC and 23\% under diffusion. We report this only to characterize how
the compressed diffusion success scale relates to the absolute differences
in Table~\ref{tab:crossarch_tests}; the diffusion interval admits zero, and
nothing here is evidence that the two effects are equal.

Second, jerk reverses sign. Under BC it is indistinguishable from both
baselines ($p \geq 0.06$); under diffusion it is better than both by $+0.096$
and $+0.092$, and both comparisons survive correction. This is the largest
effect in the diffusion grid and it points the opposite way from this
paper's headline: at least one crude trajectory statistic, never proposed as
a curation metric, has genuine curation value under the right policy
class. It is
also the second time in this paper that jerk outperforms the metrics
motivated against it, the first being detection inside a duration-matched
band. We do not have an account of why the architectures differ here, and we
...
on the mixed-quality pools the curation comparison runs on. Our
earlier reduced-compute pilot showed a similar gap, which we attributed at
the time to under-convergence; that explanation no longer holds, and we do
not have a replacement for it. The consequence for reading
Table~\ref{tab:crossarch} is concrete: the two columns are not operating at
comparable points on the success scale, so cross-architecture comparisons of
absolute differences are weaker than within-column comparisons, and the
claims above are deliberately about ordering and sign rather than magnitude.
...
For a post-training pipeline the practical reading is that a curation metric
cannot be selected once and reused across adaptation setups. The same filter
that is merely inert under one policy class is the best-performing choice
under another, and the metric that is most harmful under one is not
measurably harmful under the other. Curation and policy class are a joint
choice, and reporting a curation result without naming the architecture it
was measured on leaves out something that changes the answer.
...
cloning and a diffusion policy); the detection results are policy-free.
The cross-architecture grid of Section~\ref{sec:crossarch} carries three
...
worst under both architectures is made over a five-condition subset on the
diffusion side and a seven-condition grid on the BC side. It uses five seeds
rather than ten, which resolves differences of roughly 0.054; TED's
diffusion deficit ($-0.028$, CI $[-0.052, +0.000]$) falls below that, so we
report TED's ordering as replicating and its harm as established under BC
only. And absolute success rates remain lower under diffusion in every
condition at a matched epoch and evaluation budget, from $1.6\times$ on the
oracle (0.544 vs.\ 0.892) to $3.1\times$ on the two baselines. An earlier reduced-compute pilot showed the same gap and
we attributed it to under-convergence; restoring the full epoch and rollout
budget did not close it, and we have no replacement explanation. Because the two grids sit at different points on
the success scale, we restrict cross-architecture claims to ordering and
sign rather than magnitude. Multiple-comparison correction is applied within
each architecture, as each grid is a separate family; pooling all twelve
tests ($\alpha = 0.00417$) leaves every reported conclusion unchanged.
...
both architectures, though at five seeds its deficit there is not resolvable,
and reverses jerk from inert to better than both baselines. For a
post-training pipeline this is the operative point: which demonstrations are
worth keeping is not a property of the demonstrations alone, and a curation
metric carried unchanged across policy classes can change sign.
