...
under a diffusion policy leaves TED ranked worst under both architectures,
though at five seeds its deficit there is no longer resolvable, and reverses
jerk from inert to significantly better than both baselines, so the curation
decision does not transfer unchanged across policy classes.
...
\subsection{Cross-architecture replication under a diffusion policy}
\label{sec:crossarch}

The grid above uses a single policy class, so on its own it cannot
distinguish a property of the metric from a property of behavior cloning. We
therefore replicated it under a diffusion policy on the same curated
subsets: same keep fraction, same 1950-demonstration pools, same 600 epochs
of 500 gradient steps, same learning rate ($10^{-4}$), and the same
evaluation protocol (50 rollouts, horizon 400, every 150 epochs, best
checkpoint per seed). The policy is a conditional U-Net ($[256, 512, 1024]$
down-dims, kernel 5, 8 groups, 256-dimensional diffusion-step embedding)
with EMA (power 0.75), an observation horizon of 2, action horizon of 8, and
prediction horizon of 16, using a DDIM noise scheduler with 10 inference
steps. Batch size (256 vs.\ 100) and the sequence modeling are inherent to
the architecture and are the only respects in which the two grids differ by
more than the policy class. We ran five conditions (TED, jerk, and the three
references) with five seeds each; SAL and path length were not rerun, so the
diffusion comparison covers two of the four metrics.
...
Three results follow, and they do not all point the same way.

First, TED is the worst condition under both architectures (0.240 under BC,
0.092 under diffusion). That is what one expects if the deficit belongs to
the metric rather than to behavior cloning, and it is the part of the
original finding the replication supports. But the harm is statistically
robust under BC only. Under diffusion the deficit is $-0.024$ against random
and $-0.028$ against no curation, with intervals that admit zero, and
neither survives correction. This is a statement about power rather than a
refutation: at five seeds this grid resolves differences of roughly 0.054,
essentially the same threshold we report for the BC grid, and TED's deficit
falls below it. The defensible claim is that TED's \emph{ordering} holds
across architectures while its \emph{harm} is demonstrated under BC only.
Descriptively, and not as a tested comparison, the deficit is of broadly
similar relative size in the two grids: 37\% of the no-curation baseline
under BC and 23\% under diffusion. We report this to characterize how the
compressed diffusion success scale relates to the absolute differences in
Table~\ref{tab:crossarch_tests}, not as evidence that the two effects are
equal.

Second, jerk reverses sign. Under BC it is indistinguishable from both
baselines ($p \geq 0.06$); under diffusion it beats both by $+0.096$ and
$+0.092$, and both comparisons survive correction. This is the largest
effect in the diffusion grid, and it runs opposite to the paper's headline:
at least one crude trajectory statistic, never proposed as a curation
metric, has genuine curation value under the right policy class. It is also
the second time in this work that jerk outperforms the metrics motivated
against it, the first being detection inside a duration-matched band. We do
not have an account of why the architectures differ here, and report it as
an open result rather than a mechanism.

Third, absolute success rates are lower under diffusion in every condition,
by factors ranging from $1.6\times$ on the oracle (0.544 vs.\ 0.892) to
$3.1\times$ on the two baselines, at a matched epoch and evaluation budget
in which the diffusion runs in fact saw more samples per epoch (batch 256
vs.\ 100). The gap is therefore not a uniform offset: it is widest exactly
on the mixed-quality pools the curation comparison runs on. An earlier reduced-compute pilot showed a
similar gap, which we attributed at the time to under-convergence; that
explanation no longer holds and we have no replacement for it. The
consequence for reading Table~\ref{tab:crossarch} is concrete: the two
columns are not operating at comparable points on the success scale, so
cross-architecture comparisons of absolute differences are weaker than
within-column comparisons, and the claims above are deliberately about
ordering and sign rather than magnitude.
...
Taken together, the replication supports the metric-level reading of the TED
result and simultaneously narrows the scope of any claim about smoothness
metrics as a class. Which demonstrations are worth keeping is not a property
of the demonstrations alone, and a curation result reported without naming
the policy class it was measured on omits something that can change the
sign of the answer.
...
LIBERO. Our downstream results cover two policy classes (GMM behavior
...
path length were not rerun under diffusion, so the claim that TED ranks
...
under-convergence in a reduced-compute pilot; restoring the full epoch and
rollout budget did not close it, so that explanation was wrong and we have
none to replace it. Because the two grids sit at different points on the
success scale, we restrict cross-architecture claims to ordering and sign
rather than magnitude. Multiple-comparison correction is applied within each
architecture, as each grid is a separate family; pooling all twelve tests
($\alpha = 0.00417$) leaves every reported conclusion unchanged.
...
Replicating the grid under a diffusion policy leaves TED ranked worst under
both architectures, supporting a metric-level rather than architecture-level
reading of that result, though at five seeds its deficit there is not
resolvable and the harm remains established under behavior cloning only. The
same replication reverses jerk from inert to better than both baselines. A
curation metric's value, and its sign, depend on the policy class it is
measured against, which is not currently part of how such results are
reported.
