...
about ordering, not harm; its absolute rates stay below BC in every
condition at a matched epoch and evaluation budget, from $1.6\times$ on the
oracle to $3.1\times$ on the baselines, which rules out the
under-convergence explanation we gave for the same gap in an earlier pilot
and leaves it unexplained. Corrections are applied within each architecture; pooling all
